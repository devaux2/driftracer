...
### Verticality & risk shortcuts

Control points carry a **Y elevation**, so the road climbs hills, drops into
dips and crests — the road mesh, rails and hover physics all follow it. Jump
pads are **ramps** (their `power` scales the launch). While airborne the ship
**ignores the track walls** and only lands if it comes down *over the road* —
so a ramp at the lip of a chasm becomes a **risk shortcut**: clear the gap to
skip a whole section, come up short and you fall in and respawn (losing your
speed). The demo track's `neon-circuit` has one such chasm jump plus a banked
hill climb and a valley dip.

### Fullscreen

The game requests **fullscreen** on the first tap/click/key and again on RACE,
and locks to **landscape** on mobile where supported (both best-effort — they
no-op where the browser disallows it).
...
- [x] Verticality (elevation between points) + risk shortcuts (ramp jumps + fall/respawn)
- [x] Fullscreen on open + landscape lock
